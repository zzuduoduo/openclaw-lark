...
- 交互式读取默认使用用户身份工具，只能读取用户有权限的会话
- 定时任务、后台任务、无用户 OAuth 上下文时，读取当前群消息优先使用 `feishu_im_bot_get_messages`
- `feishu_im_user_get_messages` 中 `open_id` 和 `chat_id` 必须二选一
- `feishu_im_bot_get_messages` 只能读取当前群/任务来源群，不支持跨群任意读取
...
| 定时任务/后台任务读取当前群历史消息 | feishu_im_bot_get_messages         | 无；必要时 chat_id=当前群 oc_xxx              | relative_time, start_time/end_time, page_size, sort_rule                  |
...
`feishu_im_user_get_messages`、`feishu_im_bot_get_messages` 和 `feishu_im_user_search_messages` 支持时间过滤，话题消息不支持。
...
后台定时任务没有当前用户 OAuth 上下文，应使用机器人身份读取当前群：

```json
{ "relative_time": "today", "page_size": 50 }
```
